if (ENABLE_SANITIZERS OR ENABLE_UBSan OR ENABLE_MSAN OR ENABLE_ASAN OR ENABLE_TSan)
    ...
    elseif (MINGW) #  OR CYGWIN?
        ...
        if (CMAKE_C_COMPILER_ID STREQUAL "Clang" OR CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
            if (ENABLE_MSAN)
                message("- MSAN Enabled in CMakeLists.txt")
                set(SANITIZE_MEMORY ON)
            elseif (ENABLE_ASAN)
                message("- ASAN Enabled in CMakeLists.txt")
                set(SANITIZE_ADDRESS ON)
            elseif (ENABLE_TSan)
                message("-  TSAN Enabled in CMakeLists.txt")
                set(SANITIZE_THREAD ON)
            endif ()
        elseif (CMAKE_C_COMPILER_ID STREQUAL "GNU" OR CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
            if (ENABLE_MSAN)
                message(WARNING "GCC does not have a MSAN (SANITIZE_MEMORY).")
            elseif (ENABLE_ASAN)
                message("- ASAN Enabled in CMakeLists.txt")
                set(SANITIZE_ADDRESS ON)
            elseif(ENABLE_TSan)
